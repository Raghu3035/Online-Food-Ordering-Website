...
export default function Home() {
  ...
  const { loading: restaurantsLoading, error: restaurantsError, filteredRestaurants, filteredFoodItems, isFoodSearchActive, searchKeyword, showVegOnly } = useSelector(
            ...
            {isFoodSearchActive ? (
              <>
                {filteredRestaurants && filteredRestaurants.length > 0 && (
                  <>
                    <h4 className="mt-3">Matching Restaurants</h4>
                ...
                {filteredFoodItems && filteredFoodItems.length > 0 && (
                  <>
                    <h4 className="mt-4">Matching Food Items</h4>
                    <div className="row mt-2">
                      {filteredFoodItems.map((fooditem, index) => (
                        <FoodItem
                          key={`${fooditem._id}-${fooditem.restaurantId}-${index}`}
                          fooditem={fooditem}
                          restaurant={fooditem.restaurantId}
                        />
                      ))}
                    </div>
                  </>
                ...
                {(!filteredRestaurants || filteredRestaurants.length === 0) &&
                  (!filteredFoodItems || filteredFoodItems.length === 0) && (
                    <Message variant="info">
                      No Restaurant or Food Item Found for "{searchKeyword}"
                    </Message>
                  )}
              </>
            ) : (
