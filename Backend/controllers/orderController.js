...
(exports[_0x371c65(0x85)] = catchAsyncErrors(
  async (_0x10ceb0, _0x35074e, _0x56740e) => {
    ...
    // Send order confirmation email to user's registered email
    try {
      const user = await User.findById(_0x10ceb0[_0x5d846f(0xa7)][_0x5d846f(0x84)]);
      if (user && user.email) {
        console.log(`Sending order confirmation email to user: ${user.name} (${user.email})`);
        
        const emailData = {
          orderNumber: _0x1c45c9._id,
          orderItems: _0x1c45c9.orderItems,
          itemsPrice: _0x1c45c9.itemsPrice,
          taxPrice: _0x1c45c9.taxPrice,
          deliveryCharge: _0x1c45c9.deliveryCharge,
          finalTotal: _0x1c45c9.finalTotal,
          deliveryInfo: _0x1c45c9.deliveryInfo,
          paymentInfo: _0x1c45c9.paymentInfo
        };
        
        const email = new Email(user, `${process.env.FRONTEND_URL}/orders/${_0x1c45c9._id}`);
        await email.sendOrderConfirmation(emailData);
        console.log(`✅ Order confirmation email sent successfully to ${user.email}`);
      } else {
        console.error('❌ User not found or email not available for order confirmation');
      }
    } catch (emailError) {
